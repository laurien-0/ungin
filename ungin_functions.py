...
def analyse_confidence(input_dir, df, scale, prediction):
    """
    ...
    Parameters:
        ...
        scale (Boolean)    : True/False to indicate whether color scale bar should be included.
        prediction (string): Options are monomer, ye or hsv1.
    ...
    if prediction != "monomer":
        heading_1 = "Mean pLDDT " + prediction + " only"
        heading_2 = "Mean pLDDT without " + prediction
        ...
        if file.endswith(".pdb"):
            ...
            with open(f"structures/{file}", "r") as f:
                ...
                for line in f:
                    ...
                    if line_s[0] != "ATOM":
                        continue
                    all_plddts.append(float(line_s[-2]))
                    res_index.append(int(line[22:26]))
            
            mean_plddt = sum(all_plddts) / len(all_plddts)
            heading = "Mean pLDDT: " + prediction
            df.loc[df['ID'] == gene_code, heading] = mean_plddt
            
            if prediction == "ye":
                heading_1 = "..." + prediction + " only"
                heading_2 = "...without " + prediction
                ung_only_mean_plddt = sum(all_plddts[:1799]) / 1799
                df.loc[df['ID'] == gene_code, heading_1] = ung_only_mean_plddt
                prot_only_mean_plddt = sum(all_plddts[1799:]) / (len(all_plddts)-1799)
                df.loc[df['ID'] == gene_code, heading_2] = prot_only_mean_plddt
            if prediction == "hsv1":
                heading_1 = "..." + prediction + " only"
                heading_2 = "...without " + prediction
                ung_only_mean_plddt = sum(all_plddts[:1927]) / 1927
                df.loc[df['ID'] == gene_code, heading_1] = ung_only_mean_plddt
                prot_only_mean_plddt = sum(all_plddts[1927:]) / (len(all_plddts)-1927)
                df.loc[df['ID'] == gene_code, heading_2] = prot_only_mean_plddt
